        ...
        <div className="flex w-full gap-2 h-[30vh]">
            <Tabs defaultValue="vehicle" className="w-full">
                <TabsList className="w-full">
                    <TabsTrigger className="w-full" value="vehicle">Vehicle Details</TabsTrigger>
                    <TabsTrigger className="w-full" value="order">Order Details</TabsTrigger>
                    <TabsTrigger className="w-full" value="history">Order History</TabsTrigger>
                </TabsList>
                <TabsContent value="vehicle">
                    <div className="flex items-center justify-around h-[25vh] px-6 gap-6">
                        <div className="flex flex-col gap-2 items-center">
                            <h1 className="text-xl font-bold text-primary">{card.vehicle.space}%</h1>
                            <div className="text-background bg-primary/40 p-4 rounded-lg" style={{ backgroundImage: `linear-gradient(90deg, hsl(var(--primary)) ${card.vehicle.space}%, transparent ${card.vehicle.space}%)` }} >
                                <TruckIcon size={64} />
                            </div>
                        </div>
                        <div className="bg-slate-200/70 dark:bg-slate-800/70 p-4 rounded-2xl grid grid-cols-2 gap-4 w-full">
                            ...
                            <div className="flex flex-col">
                                ...
                                <h1 className=" text-sm font-bold">{card.vehicle.space}/100%</h1>
                            </div>
                            <div className="flex flex-col">
                            ...
                            <div className="flex flex-col">
                                ...
                                <h1 className=" text-sm font-bold">{card.vehicle.volume} in*</h1>
                            </div>
                        </div>
                    </div>
                </TabsContent>
                <TabsContent value="order">
                    <div className="bg-slate-200/70 dark:bg-slate-800/70 p-4 rounded-2xl grid grid-cols-2 gap-4 w-full h-[24vh]">
                        ...
                        <div className="flex flex-col">
                            ...
                            <h1 className=" text-sm font-bold">{card.order.quantity} Units</h1>
                        </div>
                        <div className="flex flex-col">
                        ...
                        {card.checkpoints.map((item, index) => (
                            <>
                                <div className="flex flex-col justify-center items-center">
                                    <circle className={cn("size-2 rounded-full", index === 0 || card.checkpoints[index - 1].progress === 100 ? "bg-primary" : "bg-accent-foreground")} />
                                    <div className="w-0.5 min-h-[70px] h-full flex bg-accent-foreground" style={{ backgroundImage: `linear-gradient(180deg, hsl(var(--primary)) ${item.progress}%, transparent ${item.progress}%)` }} />
                                    {index === card.checkpoints.length - 1 && <circle className={cn("size-2 rounded-full", card.checkpoints[index].progress === 100 ? "bg-primary" : "bg-accent-foreground")} />}
                                </div>
                                <div className="flex flex-col px-2">
                                    <h1 className="text-sm font-bold">{item.title}</h1>
                                    <p className="font-medium opacity-60 text-xs">{getDate(item.date)}</p>
                                </div>
                                <p className="font-medium opacity-60 text-[10px] py-2 pr-2 pl-5">11:28</p>
                            </>
                        ))}
                    </div>
                </ScrollArea>
