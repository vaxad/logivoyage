...
class MMT:
    ...
    def __init__(self, framework='DOCPLEX'):
        ...
        if framework not in ['CVXPY', 'DOCPLEX']:
            raise ValueError(
                'Framework not supported, the model only supports CVXPY and DOCPLEX')
        else:
    ...
    def set_param(self, route, order):
        ...
        self.indexPort = dict(
            zip(self.portIndex.values(), self.portIndex.keys()))
        ...
        DateList = list(route['Weekday'].replace(
            weekdayDateList).apply(json.loads))

        self.goods = order.shape[0]
        self.tranCost = np.ones(
            [self.portSpace, self.portSpace, self.dateSpace]) * bigM
        self.tranFixedCost = np.ones(
            [self.portSpace, self.portSpace, self.dateSpace]) * bigM
        self.tranTime = np.ones(
            [self.portSpace, self.portSpace, self.dateSpace]) * bigM

        for i in range(route.shape[0]):
            self.tranCost[source[i], destination[i],
                          DateList[i]] = route['Cost'][i]
            self.tranFixedCost[source[i], destination[i],
                               DateList[i]] = route['Fixed Freight Cost'][i]
            self.tranTime[source[i], destination[i],
                          DateList[i]] = route['Time'][i]
        ...
        self.whCost = np.array(self.whCost.sort_values(
            by='index')['Warehouse Cost'])
        self.kVol = np.array(order['Volume'])
        self.kValue = np.array(order['Order Value'])
        self.kDDL = np.array(
            (order['Required Delivery Date'] - self.minDate).dt.days)
        ...
        self.kStartTime = np.array(
            (order['Order Date'] - self.minDate).dt.days)
        self.taxPct = np.array(order['Tax Percentage'])

        # add available route indexes
        self.route_num = route[['Source', 'Destination']
                               ].drop_duplicates().shape[0]
        routes = route[['Source', 'Destination']
                       ].drop_duplicates().replace(self.indexPort)
        self.available_routes = list(
            zip(routes['Source'], routes['Destination']))
        # localization variables of decision variables in the matrix
        var_location = product(self.available_routes, range(
            self.dateSpace), range(self.goods))
    ...
    def cvxpy_build_model(self):
        ...
        self.var = cp.Variable(
            self.route_num * self.dateSpace * self.goods, boolean=True, name='x')
        ...
        self.var_2 = cp.Variable(
            self.route_num * self.dateSpace, integer=True, name='y')
        self.y = np.zeros((self.portSpace, self.portSpace,
                          self.dateSpace)).astype('object')
        self.y[self.var_2_location] = list(self.var_2)
        # 3 dimensional route usage matrix
        self.var_3 = cp.Variable(
            self.route_num * self.dateSpace, boolean=True, name='z')
        self.z = np.zeros((self.portSpace, self.portSpace,
                          self.dateSpace)).astype('object')
        self.z[self.var_3_location] = list(self.var_3)
        # warehouse related cost
        warehouseCost, arrTime, stayTime = self.warehouse_fee(self.x)
        ### objective###
        transportCost = np.sum(self.y * self.tranCost) + \
            np.sum(self.z * self.tranFixedCost)
        transitDutyCost = np.sum(
            np.sum(np.dot(self.x, self.kValue), axis=2) * self.transitDuty)
        ...
        constraints += [np.sum(self.x[self.kStartPort[k], :, :, k])
                        == 1 for k in range(self.goods)]
        constraints += [np.sum(self.x[:, self.kEndPort[k], :, k])
                        == 1 for k in range(self.goods)]
        # 2.For each goods k, it couldn't be shipped out from its destination or shipped to its origin.
        constraints += [np.sum(self.x[:, self.kStartPort[k], :, k])
                        == 0 for k in range(self.goods)]
        constraints += [np.sum(self.x[self.kEndPort[k], :, :, k])
                        == 0 for k in range(self.goods)]
        ...
        constraints += [stayTime[j, k] >=
                        0 for j in range(self.portSpace) for k in range(self.goods)]
    ...
    def cplex_build_model(self):
        ...
        model = Model()
        # 4 dimensional binary decision variable matrix
        self.var = model.binary_var_list(
            self.route_num * self.dateSpace * self.goods, name='x')
        self.x = np.zeros((self.portSpace, self.portSpace,
                          self.dateSpace, self.goods)).astype('object')
        self.x[self.var_location] = self.var
        # 3 dimensional container number matrix
        self.var_2 = model.integer_var_list(
            self.route_num * self.dateSpace, name='y')
        self.y = np.zeros((self.portSpace, self.portSpace,
                          self.dateSpace)).astype('object')
        self.y[self.var_2_location] = self.var_2
        # 3 dimensional route usage matrix
        self.var_3 = model.binary_var_list(
            self.route_num * self.dateSpace, name='z')
        self.z = np.zeros((self.portSpace, self.portSpace,
                          self.dateSpace)).astype('object')
        ...
        model.add_constraints(
            np.sum(self.x[self.kStartPort[k], :, :, k]) == 1 for k in range(self.goods))
        model.add_constraints(
            np.sum(self.x[:, self.kEndPort[k], :, k]) == 1 for k in range(self.goods))
        # 2.For each goods k, it couldn't be shipped out from its destination or shipped to its origin.
        model.add_constraints(
            np.sum(self.x[:, self.kStartPort[k], :, k]) == 0 for k in range(self.goods))
        model.add_constraints(
            np.sum(self.x[self.kEndPort[k], :, :, k]) == 0 for k in range(self.goods))
        # 3.constraint for transition point
        for k in range(self.goods):
            for j in range(self.portSpace):
                if (j != self.kStartPort[k]) & (j != self.kEndPort[k]):
                    model.add_constraint(
                        np.sum(self.x[:, j, :, k]) == np.sum(self.x[j, :, :, k]))
        # 4.each goods can only be transitioned in or out of a port for at most once
        model.add_constraints(np.sum(self.x[i, :, :, k]) <= 1 for k in range(
            self.goods) for i in range(self.portSpace))
        model.add_constraints(np.sum(self.x[:, j, :, k]) <= 1 for k in range(
            self.goods) for j in range(self.portSpace))
        # 5.transition-out should be after transition-in
        model.add_constraints(stayTime[j, k] >= 0 for j in range(
            self.portSpace) for k in range(self.goods))
        # 6.constraint for number of containers used
        numCtn = np.dot(self.x, self.kVol) / self.ctnVol
    ...
    def solve_model(self, solver=cp.CBC):
        ...
        try:
            if self.framework == 'CVXPY':
                ...
                self.xs = np.zeros(
                    (self.portSpace, self.portSpace, self.dateSpace, self.goods))
                self.xs[self.var_location] = self.var.value
                self.ys = np.zeros(
                    (self.portSpace, self.portSpace, self.dateSpace))
            ...
            raise Exception(
                'Model is not solvable, no solution will be provided')
        ...
        nonzeroX = list(map(lambda x: (self.portIndex[x[0]], self.portIndex[x[1]],
                                       (self.minDate +
                                        pd.to_timedelta(x[2], unit='days')).date().isoformat(),
                                       x[3]), nonzeroX))

        self.whCostFinal, arrTime, _ = self.warehouse_fee(self.xs)
        self.transportCost = np.sum(
            self.ys * self.tranCost) + np.sum(self.zs * self.tranFixedCost)
        ...
        for i in range(self.goods):
            self.solution_[
                'goods-' + str(i + 1)] = list(filter(lambda x: x[3] == i, nonzeroX))
    ...
    def warehouse_fee(self, x):
        ...
        startTime = np.arange(self.dateSpace).reshape(
            1, 1, self.dateSpace, 1) * x
        ...
        stayTime = np.sum(startTime, axis=(1, 2)) - \
            np.sum(arrTimeMtrx, axis=(0, 2))
        stayTime[self.kStartPort.tolist(), range(self.goods)
                 ] -= self.kStartTime
        warehouseCost = np.sum(
            np.sum(stayTime * self.kVol, axis=1) * self.whCost)
    ...
    def txt_solution(self, route, order):
        ...
        txt += "\nTotal cost: " + \
            str(self.transportCost + self.whCostFinal + self.taxCost)
        ...
        for i in range(order.shape[0]):
            ...
            txt += "\nArrival date: " + \
                str(self.arrTime_['goods-' + str(i + 1)])
            txt += "\nRoute:"
...
def transform(filePath):
    ...
    route = route[list(route.columns[0:4]) + ['Fixed Freight Cost', 'Time',
        ...
        route.columns[-9:-2])]
    route = pd.melt(
        route, id_vars=route.columns[0:10], value_vars=route.columns[-7:], var_name='Weekday', value_name='Feasibility')
...
if __name__ == '__main__':
    order, route = transform("data.xlsx")
    m = MMT()
